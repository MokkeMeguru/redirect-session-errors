...
The `home-routes` handler in the `redirect-session-errors.routes.home` namespace
...
The `home-page` function will in turn call the `redirect-session-errors.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/redirect_session_errors/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `redirect-session-errors.handler` namespace:
...
Request middleware functions are located under the `redirect-session-errors.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `redirect-session-errors.dev-middleware` namespace found in
